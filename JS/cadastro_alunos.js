function cadastrarAlunos(e) {
    ...
    const regex_senha = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{6,10}$/;

    if (serie === '' || turma === '' || nome === '' || senha === ''){
    ...
    const dadosFormulario = { serie, turma, nome, senha };
    console.log('Front-end: Tentando enviar os dados para o servidor...', dadosFormulario);

    fetch('http://localhost:3000/cadastrar_alunos', {
        method: 'POST',
        headers: {
...
if (formulario) {
    formulario.addEventListener('submit', cadastrarAlunos);
}
